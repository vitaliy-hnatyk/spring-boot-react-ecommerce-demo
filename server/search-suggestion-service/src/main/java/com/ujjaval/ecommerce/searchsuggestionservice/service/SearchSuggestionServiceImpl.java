...
public class SearchSuggestionServiceImpl implements SearchSuggestionService {
    HashMap<String, List<InnerSearchSuggestionKeywordInfo>> prefixKeywordsMap = new HashMap<>();
    List<InnerSearchSuggestionKeywordInfo> defaultSearchSuggestionList = new LinkedList<>();
    ...
    private void addJsonObjKeywordToMap(JSONObject jsonResponse, String key, String attributeName) throws JSONException {
        ...
        for (int index = 0; index < jsonArray.length(); ++index) {
            ...
            if (index < 10 && attributeName.equals("apparels")) {
                defaultSearchSuggestionList.add(new InnerSearchSuggestionKeywordInfo(jsonObject.getString("type"),
                        filterLink, 1));
            }
        }
    }
        ...
        for (int index = 0; index < keyword.length(); ++index) {
            ...
            if (!prefixKeywordsMap.containsKey(prefix)) {
                prefixKeywordsMap.put(prefix, new ArrayList<>(Arrays.asList(
                        new InnerSearchSuggestionKeywordInfo(keyword, link, 1))));
            } else {
                List<InnerSearchSuggestionKeywordInfo> keywordList = prefixKeywordsMap.get(prefix);
                keywordList.add(new InnerSearchSuggestionKeywordInfo(keyword, link, 1));
            }
        }
    }
    ...
    public List<InnerSearchSuggestionKeywordInfo> searchKeywordFromMap(String q) {
        List<InnerSearchSuggestionKeywordInfo> resultList = null;
    ...
    public List<InnerSearchSuggestionKeywordInfo> getDefaultSearchSuggestions() {
